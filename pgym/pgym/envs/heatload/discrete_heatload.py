...
class DiscreteHeatLoadEnv(gym.Env):
    ...
    def get_observation_space(self):
        def get_obs_bound(case0):
            ...
            pgm = case0['gen'][:, PMAX]
            pdm = np.array([p * 5 for p in case0['bus'][:, PD]])
            trm = case0['load'][:, TMAX]
            tam = case0['load'][:, TMAX]
            plm = case0['load'][:, LMAX]
    ...
    def get_action_space(self):
        ...
        return gym.spaces.Discrete(2 ** cnt), min_action, max_action

    # return case
    def put_action(self, action):
        case = self.case
        action_temp = action
        for k in range(len(self.case0['load'][:, 0])):
            sign = 1 - 2 * (action_temp % 2)
            action_temp //= 2
            tov = sign * self.action_scale[k] + \
                case['load'][case['controlled_load'][k, 0], PL]
            tov = max([tov, self.min_action[k]])
            tov = min([tov, self.max_action[k]])
            case['load'][case['controlled_load'][k, 0], PL] = tov
        return case
